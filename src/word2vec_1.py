...
pd.concat([pd.DataFrame({'CLNT_ID': np.sort(train['CLNT_ID'].unique())}), train_features], axis=1).to_csv(word2vec_path +'X_train_w2v_CLAC3_NM.csv', index=False)
pd.concat([pd.DataFrame({'CLNT_ID': np.sort(test['CLNT_ID'].unique())}), test_features], axis=1).to_csv(word2vec_path + 'X_test_w2v_CLAC3_NM.csv', index=False)
